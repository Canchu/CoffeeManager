...
def main():
	...
	for user in users:
		...
		for drink in drinks:
			bills[user][drink] = []
			sql = "select count(*), coalesce(sum(value), 0) from test where name = '%s' and item = '%s';" % ( user, drink )
			cursor.execute(sql)
			results = cursor.fetchall()
			for row in results:
				bills[user][drink].append(int(row[0]))
				bills[user][drink].append(int(row[1]))
		sql = "select count(*), coalesce(sum(value), 0) from test where name = '%s';" % user
		cursor.execute(sql)
